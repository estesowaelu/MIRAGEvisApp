...
public:
	...
	void update(float dt)
	{
		...
		Vec3 targetVelocity = Vec3((mStrafeRight - mStrafeLeft), (mRaise-mLower), (mUp - mDown)) * mFPMaxSpeed;
	...
	void mouseMotion(int x, int y)
	{
		...
		if((mRightClick && mLeftClick) || mMiddleClick)
		{
			...
			Vec3 up(sinf(yaw)*sinf(pitch), cosf(pitch), cosf(yaw)*sinf(-pitch));
...
//		return getViewMatrix(FPS_VIEW).m;
	...
	void setRaise(bool r)	{mRaise = r;}
	void setLower(bool l)	{mLower = l;}
	...
	bool mUp, mDown, mLeft, mRight, mStrafeLeft, mStrafeRight, mRaise, mLower;
};
